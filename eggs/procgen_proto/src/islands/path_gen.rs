...
#[derive(Component)]
pub struct PathPlacement(pub HashMap<IVec2, PathKind>);
...
pub fn generate_path_placements(
    ...
    let elevation_noise = island_gen.base_elevation_noise;

    let mut path_cells = HashMap::new();

    for (start_region_id, end_region_id) in path_edges {
        let start_region = &regions[start_region_id];
        let start_pos = start_region.center + start_region.feature_offset;

        let end_region = &regions[end_region_id];
        let end_pos = end_region.center + end_region.feature_offset;
        ...
        while path_pos != end_pos {
            ...
            for neighbor_pos in neighbors {
                ...
                if regions.contains_key(&neighbor_region_id) {
                    ...
                    let elevation_change = neighbor_elevation - path_elevation;

                    // add some extra randomness to make paths less boring?
                    let mut score = dist_to_end + elevation_change;
